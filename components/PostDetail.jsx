...
const PostDetail = ({post}) => {
    const getContentFragment = (index, text, obj, type) => {
        ...
        switch (type) {
          ...
          case 'heading-six':
            return <h6 key={index} className="text-sm font-light mb-4">{modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)}</h6>;
            // case 'class':
            //     console.log(obj.children[0].children[0].text)

            //     return <p key={index} className={obj.className + ' mb-4'}>{obj.children[0].children[0].text}</p>;
